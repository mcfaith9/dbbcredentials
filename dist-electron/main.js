import require$$1$3, { ipcMain, Notification, shell, app, BrowserWindow, dialog } from "electron";
...
function setupAutoUpdater() {
  ...
  main$1.autoUpdater.on("update-available", (info) => {
    ...
    updateState.error = null;
    updateState.lastChecked = (/* @__PURE__ */ new Date()).toISOString();
    broadcastUpdateStatus();
  });
  ...
  main$1.autoUpdater.on("error", (err) => {
    ...
    updateState.error = (err == null ? void 0 : err.message) || "Failed to check for updates";
    updateState.lastChecked = (/* @__PURE__ */ new Date()).toISOString();
    broadcastUpdateStatus();
  });
...
ipcMain.handle("dialog:saveVaultFile", async (_event, payload) => {
  ...
  try {
    ...
    await fs$j.writeFile(filePath, payload.data, "utf-8");
...
ipcMain.handle("dialog:openVaultFile", async (_event, payload) => {
  ...
  try {
    ...
    return { success: true, filePath, content, fileName: path$n.basename(filePath) };
  } catch (err) {
...
ipcMain.handle("updater:check", async () => {
  ...
  try {
    ...
    return {
      success: false,
      error: (err == null ? void 0 : err.message) || "Failed to check for updates",
      currentVersion: app.getVersion(),
      isPackaged: true
    };
  }
...
ipcMain.handle("updater:download", async () => {
  ...
  try {
    ...
    updateState.status = "error";
    updateState.error = (err == null ? void 0 : err.message) || "Download failed";
    broadcastUpdateStatus();
    return { success: false, error: (err == null ? void 0 : err.message) || "Failed to download update" };
  }
});
ipcMain.handle("updater:install", () => {
  ...
  try {
    ...
    return { success: false, error: (err == null ? void 0 : err.message) || "Failed to install update" };
  }
});
